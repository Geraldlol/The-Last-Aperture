...
activates_on:
  ...
  signals:
    - any_of: ['cryptography', 'pycryptodome', 'Crypto.Cipher', 'hashlib', 'hmac', 'secrets', 'PyJWT', 'import jwt', 'authlib', 'passlib', 'bcrypt', 'argon2-cffi', 'PyNaCl', 'nacl', 'pyOpenSSL', 'python3-saml', 'xmlsec']
    - any_of: ['node:crypto', 'crypto.createCipheriv', 'crypto.timingSafeEqual', 'jsonwebtoken', 'jose', 'jwks-rsa', 'node-forge', 'bcryptjs', 'argon2', 'libsodium-wrappers', 'tweetnacl', '@node-saml/node-saml', 'openid-client']
    - any_of: ['javax.crypto.Cipher', 'SecretKeySpec', 'IvParameterSpec', 'java.security.SecureRandom', 'MessageDigest', 'Signature', 'SSLContext', 'X509TrustManager', 'org.bouncycastle', 'com.auth0:java-jwt', 'nimbus-jose-jwt', 'opensaml']
    - any_of: ['crypto/rand', 'crypto/aes', 'crypto/cipher', 'crypto/hmac', 'crypto/subtle', 'subtle.ConstantTimeCompare', 'hmac.Equal', 'golang-jwt/jwt', 'go-jose', 'golang.org/x/crypto/bcrypt', 'golang.org/x/crypto/argon2', 'golang.org/x/crypto/nacl', 'tls.Config', 'MinVersion', 'InsecureSkipVerify']
    - any_of: ['Crypto.encrypt', 'encryptWithManagedIV', 'Crypto.decrypt', 'decryptWithManagedIV', 'AES256-GCM', 'aaData', 'Crypto.generateMac', 'Crypto.generateDigest', 'Crypto.generateAesKey', 'Crypto.areEqualConstantTime', 'Crypto.sign', 'Crypto.verify', 'EncodingUtil.base64Decode', 'EncodingUtil.convertFromHex', 'RestContext.request.headers', 'Blob', 'constantTimeEquals', 'secureCompare', 'safeEquals']
    - any_of: ['CryptoKit', 'swift-crypto', 'CommonCrypto', 'SymmetricKey', 'AES.GCM', 'ChaChaPoly', 'authenticationCode', 'isValidAuthenticationCode', 'Insecure.MD5', 'Insecure.SHA1', 'SecRandomCopyBytes', 'kSecAttrAccessible', 'kSecAttrAccessGroup', 'kSecAttrAccount', 'kSecAttrService', 'kSecAttrKeyType']
    - any_of: ['System.Security.Cryptography', 'Aes.Create(', 'RandomNumberGenerator', 'Rfc2898DeriveBytes', 'ServerCertificateValidationCallback', 'openssl_encrypt', 'password_hash', 'hash_equals', 'firebase/php-jwt', 'OpenSSL::Cipher', 'ActiveSupport::MessageEncryptor', 'ring', 'aes-gcm', 'rustls', 'jsonwebtoken']
    - any_of: ['MODE_ECB', 'AES/ECB/PKCS5Padding', 'AES/CBC/PKCS5Padding', 'new IvParameterSpec(new byte[16])', 'InsecureSkipVerify: true', 'verify=False', 'rejectUnauthorized: false', 'NODE_TLS_REJECT_UNAUTHORIZED=0', 'alg":"none', '"verify_signature": False', 'jwt.decode(', '.setSeed(', 'Math.random(', 'random.random(', 'md5(', 'sha1(', 'base64']
    - any_of: ['X-Signature', 'X-Hub-Signature', 'Stripe-Signature', 'ecrecover', 'secp256k1', 'low-s', '/.well-known/jwks.json', 'SAMLResponse', 'InResponseTo', 'KeyInfo', 'argon2id$', '$2b$', 'pbkdf2_sha256$']
    - 'node:crypto'
    - 'Ed25519'
    - 'crypto-inventory'
    - 'crypto_inventory'
...
**This is the other recipe with a T2 variant, and it is not a substitute:** asserting the *deployed* policy requires reaching the real endpoint, which is never repository proof. Report the local result separately. If an accepted authenticated operator statement names the exact endpoint, scope, effect, and credential use, it authorizes an external-evidence route without another prompt; execute only through a matching credential-aware, destination-bound controller using explicitly supplied or controller-referenced material. If that route or material is absent, record the deployed policy as `UNPROVEN` with the technical gap. Authority does not conjure credential bytes, repository configuration cannot add a destination, and the external observation does not create repository proof.
