...
- **Whether an unresolved dependency name is unregistered.** That is a registry query, and this project's hard rails make the registry a remote host. See item 6 and P6: the offline half is provable, the registry half is handed to the user with one command.
...
**`A16` is the check, and its rail is the point.** It resolves every imported name against the checkout only: manifests, lockfiles, vendored directories, local modules, `node_modules`, and the runtime's builtins. It contacts no registry, because a registry is a remote host and this project's rails forbid one.
...
2. **Never install the name to find out.** Resolution runs the package manager, which runs lifecycle scripts, which is the payload. `npm install` against a name you suspect an attacker registered *is* the attack, executed by the auditor. `npm view` and `pip index` are network calls the rails forbid; hand the command to the user instead.
...
**Rejected because the check needs a registry.** "The package name is one edit away from a far more popular package" — typosquat proximity needs popularity data from a registry, which is a remote host under rail 1, and a typosquat *is* a real package so an existence check cannot refute it. The offline half survives as item 6: the name resolves nowhere in this checkout. The registry half is handed to the user with the command, and `cicd-and-supply-chain` owns the slug.
...
- The browser half is **T2 and opt-in**: render the page in a headless browser against a booted local dev server on `127.0.0.1` and assert `window.__pwned` was never set. Ask before booting anything, every time, and where the user declines, report the T1 result and record `UNPROVEN` with the reason rather than letting the absence read as safety.
...
**What is not, and how to write it:** whether the name is unregistered, and therefore claimable by an attacker. That is a registry query and the rails make a registry a remote host. Record it as the assumption, and hand the user the single command to settle it themselves — `npm view <name>` or the equivalent index query — with the warning attached: **never install it.** Installation runs lifecycle scripts, which is the payload.
