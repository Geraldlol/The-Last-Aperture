...
test('release wiring exposes fixed authenticated campaigns through the controller', () => {
  ...
  assert.match(source, /fixedCampaignOnly: true/)
  assert.doesNotMatch(source, /campaign-written|authorization-document|approver-public-key|countersignature/i)
  assert.match(source, /single-action campaign[\s\S]*campaign ledger/i)
