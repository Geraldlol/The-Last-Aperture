...
test('the v0.12 authenticated campaign is release-wired through the governing skill', () => {
  ...
  assert.match(skill, /Compose available controllers and host tools under the same attestation[\s\S]*legacy\/direct work/i)
  assert.match(skill, /For Unleash, compose only routes admitted[\s\S]*controller-policy authority/i)
  ...
  assert.match(skill, /Process\/device targets need an enrolled host adapter/i)
  ...
  assert.match(rootSkill, /target <HTTPS URL> and go` authorizes bounded HTTP recon/i)
  assert.match(rootSkill, /campaign-attested[\s\S]*scope-valid response-derived probes without repeat confirmation/i)
  assert.match(
    rootSkill,
    /For legacy\/direct work, use standard browser, process, network, Burp, Ghidra,[\s\S]*and Frida tooling under the same authority and evidence chain/i,
  )
  assert.match(rootSkill, /Unleash may use[\s\S]*only enrolled routes admitted by retained controller policy/i)
  assert.match(rootSkill, /missing (?:dedicated )?wrapper does not create another authorization gate/i)
})
